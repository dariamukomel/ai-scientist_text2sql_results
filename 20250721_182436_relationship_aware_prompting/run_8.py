...
PROMPT_DATA = {
    "system_prompt": "Думай шаг за шагом. Строго следуй этому процессу:\n\n1. Прочитай схему в формате M-schema\n2. "
                     "Перечисли используемые таблицы и колонки\n3. Проверь их наличие в схеме\n4. Сгенерируй SQL\n\nПример:\n【Schema】"
    ...
    "regen_user_prompt": " Тебе необходимо исправить SQL-запрос. Вопрос пользователя: {question} . SQL: ```sql\n {sql} ```. "
                         "Убедись, что колонки из запроса существуют в нужных таблицах.Используй ТОЛЬКО продемонстрированные в схеме и "
                         "примерах колонки. Если колонки нет в схеме или базе данных не пытайся ее подставить. В ответе укажи только "
                         "SQL-запрос, больше ничего. Исправь SQL-запрос. \\n {question} "
}
