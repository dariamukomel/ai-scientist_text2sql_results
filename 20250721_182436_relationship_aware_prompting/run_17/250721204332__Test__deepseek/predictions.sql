...
SELECT name, salary_from, salary_to  FROM vacancies  WHERE name = 'Java разработчик'    AND (salary_from > 150000 OR salary_to > 150000)    AND schedule_type = 'remote'
...
SELECT name  FROM vacancies  WHERE salary_from >= 50000    AND salary_to <= 100000    AND salary_currency = 'RUR'
...
SELECT v.name  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_street = 'Большая Морская'
...
SELECT COUNT(*)  FROM vacancies v WHERE v.company_id = 5
