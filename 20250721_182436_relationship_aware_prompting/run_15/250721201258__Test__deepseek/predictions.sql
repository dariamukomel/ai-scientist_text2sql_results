...
SELECT salary_currency, MAX(salary_to) as max_salary FROM vacancies GROUP BY salary_currency
...
SELECT      name,      level,      direction,      work_location,      salary_from,      salary_to,      salary_currency FROM      vacancies WHERE      salary_currency = 'RUR'      AND salary_from >= 50000      AND salary_to <= 100000
...
SELECT COUNT(*)  FROM vacancies  WHERE level = 'middle'    AND ((salary_from > 100000 AND salary_currency = 'RUR')         OR (salary_to > 100000 AND salary_currency = 'RUR'))
...
SELECT c.name, COUNT(v.vacancy_id) as vacancy_count FROM companies c JOIN vacancies v ON c.company_id = v.company_id WHERE c.company_city = 'Москва' GROUP BY c.name ORDER BY vacancy_count DESC LIMIT 5
...
SELECT name  FROM vacancies  WHERE salary_currency = 'USD' AND salary_from > 5000
...
SELECT name, salary_from, salary_to FROM vacancies ORDER BY (salary_from  salary_to)/2 DESC LIMIT 10
SELECT d.department_name  FROM vacancies v  JOIN departments d ON v.department_id = d.department_id  WHERE v.vacancy_id = 3
SELECT v.name  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE v.direction = 'backend' AND c.name = 'Yandex'
SELECT DISTINCT c.name  FROM companies c JOIN vacancies v ON c.company_id = v.company_id WHERE v.level = 'senior' AND c.company_city = 'Москва'
...
SELECT COUNT(*)  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE v.level = 'middle'  AND c.company_city = 'Санкт-Петербург' AND v.date_added >= '2022-01-01'  AND v.date_added < '2023-01-01'
...
SELECT COUNT(v.vacancy_id)  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_street LIKE '%Ленина%'
