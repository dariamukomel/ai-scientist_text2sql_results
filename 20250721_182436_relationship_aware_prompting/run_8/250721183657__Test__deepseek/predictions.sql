...
SELECT work_location, MAX(salary_to) as max_salary FROM vacancies WHERE work_location IN ('Москва', 'Омск', 'Екатеринбург')   AND date_added >= '2022-10-01'   AND date_added < '2022-11-01'   AND salary_to IS NOT NULL GROUP BY work_location
...
SELECT vacancies.name  FROM vacancies  JOIN companies ON vacancies.company_id = companies.company_id  WHERE companies.company_city = 'Омск' AND vacancies.schedule_type = 'flexible'
...
SELECT vacancies.name  FROM vacancies  JOIN cities ON vacancies.work_location = cities.city_name  WHERE vacancies.work_location = 'Санкт-Петербург'  AND vacancies.date_added >= '2021-02-01'  AND vacancies.date_added < '2021-03-01'
...
SELECT work_location, COUNT(*) as vacancy_count  FROM vacancies  GROUP BY work_location  ORDER BY vacancy_count DESC  LIMIT 1
...
SELECT vacancies.name  FROM vacancies  JOIN companies ON vacancies.company_id = companies.company_id  WHERE vacancies.direction = 'backend' AND companies.name = 'Yandex'
SELECT DISTINCT companies.name  FROM vacancies  JOIN companies ON vacancies.company_id = companies.company_id  WHERE vacancies.level = 'senior' AND companies.company_city = 'Москва'
...
SELECT COUNT(*)  FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id JOIN cities ON companies.company_city = cities.city_name WHERE vacancies.level = 'middle'  AND cities.city_name = 'Санкт-Петербург' AND vacancies.date_added >= '2022-01-01'  AND vacancies.date_added < '2023-01-01'
SELECT AVG(vacancies.salary_from)  FROM vacancies  JOIN companies ON vacancies.company_id = companies.company_id  JOIN cities ON companies.company_city = cities.city_name  WHERE cities.city_name = 'Екатеринбург'
