...
SELECT v.name, v.direction, v.salary_from, v.salary_to, v.salary_currency  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_street = 'Большая Морская'
...
SELECT name, salary_from, salary_to  FROM vacancies  WHERE salary_currency = 'USD' AND (salary_from > 5000 OR salary_to > 5000)
...
SELECT AVG(v.salary_from) as avg_salary_from FROM vacancies v JOIN companies c ON v.company_id = c.company_id JOIN cities ct ON c.company_city = ct.city_name WHERE ct.city_name = 'Екатеринбург'
