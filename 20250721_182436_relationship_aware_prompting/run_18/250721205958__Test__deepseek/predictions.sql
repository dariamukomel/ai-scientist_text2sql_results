...
SELECT name  FROM vacancies  WHERE salary_currency = 'RUR'    AND salary_from >= 50000    AND salary_to <= 100000
...
SELECT v.name, v.direction, v.salary_from, v.salary_to  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_street = 'Большая Морская'
SELECT COUNT(*)  FROM vacancies  WHERE level = 'middle'    AND salary_from > 100000    AND salary_currency = 'RUR'
SELECT salary_currency, COUNT(*) as currency_count  FROM vacancies  GROUP BY salary_currency  ORDER BY currency_count DESC  LIMIT 1
...
SELECT salary_currency, COUNT(vacancy_id) as vacancy_count  FROM vacancies  GROUP BY salary_currency
SELECT COUNT(*) FROM vacancies WHERE schedule_type = 'remote'
SELECT c.name, COUNT(*) as vacancy_count  FROM vacancies v  JOIN companies c ON v.company_id = c.company_id  WHERE c.company_city = 'Москва'  GROUP BY c.name  ORDER BY vacancy_count DESC  LIMIT 5
SELECT work_location, COUNT(*) as vacancy_count  FROM vacancies  WHERE work_location IS NOT NULL  GROUP BY work_location  ORDER BY vacancy_count DESC  LIMIT 1
SELECT name, salary_to  FROM vacancies  WHERE work_location = 'Москва'  ORDER BY salary_to DESC  LIMIT 5
...
SELECT name  FROM vacancies  WHERE salary_currency = 'USD'    AND (salary_from > 5000 OR salary_to > 5000)
