...
PROMPT_DATA = {
    "system_prompt": "Думай шаг за шагом. Строго следуй этому процессу:\n\n1. Прочитай схему в формате M-schema\n2. Перечисли используемые"
                     ...
                     "Используемые таблицы: sales, products\n2. Используемые колонки: products.name, sales.amount\n3. SQL: SELECT p.name,"
                     " SUM(s.amount) as total_sales FROM products p JOIN sales s ON p.id = s.product_id GROUP BY p.name\n\n【Schema】\n# "
                     "Table: sales\n[\n(id:int,PK,product_id:int,amount:decimal,date:date)\n]\n# Table: products\n[\n(id:int,PK,name:text,"
                     "price:decimal)\n]\n\nВопрос: найди продукты с общей суммой продаж > 1000\nШаги:\n1. Используемые таблицы: sales, "
                     "products\n2. Используемые колонки: products.name, sales.amount\n3. SQL: SELECT p.name, SUM(s.amount) as total_sales"
                     " FROM products p JOIN sales s ON p.id = s.product_id GROUP BY p.name HAVING SUM(s.amount) > 1000\n\n【Schema】"
                     ...
                     "SELECT EXTRACT(MONTH FROM created_at) as month, AVG(total) as avg_order FROM orders GROUP BY "
                     "EXTRACT(MONTH FROM created_at)\n\n【Schema】\n# Table: employees\n[\n(id:int,PK,name:text,manager_id:int,NULLABLE,"
                     "department:text)\n]\n\nВопрос: найди сотрудников без менеджера\nШаги:\n1. Используемые таблицы: employees\n2. "
                     "Используемые колонки: employees.name, employees.manager_id\n3. SQL: SELECT name FROM employees WHERE manager_id "
                     ...
                     "products.name, products.price, products.discontinued\n3. SQL: SELECT name FROM products WHERE price > 50 AND "
                     "(discontinued IS NULL OR discontinued = false)\n\n【Schema】\n# Table: orders\n[\n(id:int,PK,user_id:int,FK->users."
                     "id,total:decimal,created_at:date)\n]\n\nВопрос: найди заказы за последние 30 дней\nШаги:\n1. Используемые таблицы: "
                     "orders\n2. Используемые колонки: orders.id, orders.created_at\n3. SQL: SELECT id FROM orders WHERE created_at >= "
                     "CURRENT_DATE - INTERVAL '30 days'\n\n【Schema】\n# Table: events\n[\n(id:int,PK,name:text,start_date:date,end_date:"
                     "date)\n]\n\nВопрос: найди события, которые начались в январе 2023\nШаги:\n1. Используемые таблицы: events\n2. "
                     "Используемые колонки: events.name, events.start_date\n3. SQL: SELECT name FROM events WHERE EXTRACT(MONTH FROM "
                     "start_date) = 1 AND EXTRACT(YEAR FROM start_date) = 2023\n\n{hints} \n\n {ddl} {gold} {stats}",
    "user_prompt": "Строго следуй шагам:\n1. Перечисли используемые таблицы\n2. Перечисли используемые колонки\n3. Проверь их в схеме\n4. "
                   "Напиши SQL для: {question}\n\nЕсли нужно несколько таблиц:\n- Проверь связи между ними в схеме\n- Используй JOIN "
                   "только когда необходимо\n- Для JOIN давай таблицам короткие алиасы (например t1, t2)\n\nДля дат:\n- Используй "
                   "CURRENT_DATE для сегодняшней даты\n- Для интервалов используй INTERVAL 'N days'\n- Для извлечения частей даты "
                   "используй EXTRACT()\n\nФормат ответа:\n1. Таблицы: [список]\n2. Колонки: [список]\n3. SQL: [запрос]",
    "regen_system_prompt": "Ты профессиональный аналитик баз данных Postgres. Тебе необходимо исправить SQL-запрос, чтобы он выдавал "
                           "правильные данные. Результат выполнения запроса: {result} . {hints} \n\n {ddl} {stats} ",
    "regen_user_prompt": " Тебе необходимо исправить SQL-запрос. Вопрос пользователя: {question} . SQL: ```sql\n {sql} ```. Убедись, "
                         "что колонки из запроса существуют в нужных таблицах.Используй ТОЛЬКО продемонстрированные в схеме и примерах "
                         "колонки. Если колонки нет в схеме или базе данных не пытайся ее подставить. В ответе укажи только SQL-запрос, "
                         "больше ничего. Исправь SQL-запрос. \\n {question} "
}
