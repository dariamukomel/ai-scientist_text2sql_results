...
SELECT      schedule_type,      AVG((salary_from  salary_to) / 2) as average_salary  FROM      vacancies  GROUP BY      schedule_type
...
SELECT vacancy_id, name, salary_from, salary_to  FROM vacancies  WHERE salary_currency = 'USD'
...
SELECT vacancy_id, name, date_added  FROM vacancies  ORDER BY date_added DESC  LIMIT 10
