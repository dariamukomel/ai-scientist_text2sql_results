...
SELECT v.name, v.schedule_type  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_city = 'Омск' AND v.schedule_type = 'flexible'
...
SELECT vacancy_id, name  FROM vacancies  WHERE date_added = '2020-04-01'
...
SELECT v.name  FROM vacancies v  WHERE v.salary_currency = 'USD'
...
SELECT DISTINCT v.work_location  FROM vacancies v  WHERE v.schedule_type = 'remote'
...
SELECT salary_currency, COUNT(*) as count  FROM vacancies  GROUP BY salary_currency  ORDER BY count DESC  LIMIT 1
...
SELECT v.name  FROM vacancies v WHERE v.work_location = 'Санкт-Петербург'    AND v.date_added >= '2021-02-01'    AND v.date_added < '2021-03-01'
...
SELECT COUNT(DISTINCT c.company_id)  FROM vacancies v  JOIN companies c ON v.company_id = c.company_id  WHERE v.schedule_type = 'fullDay'
SELECT vacancies.salary_currency, COUNT(*)  FROM vacancies  GROUP BY vacancies.salary_currency
...
SELECT v.name  FROM vacancies v JOIN companies c ON v.company_id = c.company_id JOIN cities ci ON c.company_city = ci.city_name WHERE ci.city_name = 'Москва' ORDER BY v.salary_to DESC LIMIT 5
...
SELECT COUNT(*)  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_id = 5
SELECT COUNT(v.vacancy_id)  FROM vacancies v  JOIN companies c ON v.company_id = c.company_id  WHERE c.name = 'Yandex'
SELECT v.name, v.salary_from, v.salary_to  FROM vacancies v  WHERE v.salary_currency = 'USD'    AND (v.salary_from > 5000 OR v.salary_to > 5000)
SELECT COUNT(*) FROM vacancies WHERE date_added = CURRENT_DATE
SELECT AVG(v.salary_from)  FROM vacancies v  WHERE v.salary_currency = 'USD'
SELECT v.name  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.name = 'ООО Ништяк'
SELECT v.vacancy_id, v.name  FROM vacancies v  WHERE v.date_added > '2022-01-01'
SELECT name, salary_from, salary_to  FROM vacancies  ORDER BY salary_to DESC  LIMIT 10
SELECT d.department_name  FROM vacancies v  JOIN departments d ON v.department_id = d.department_id  WHERE v.vacancy_id = 3
SELECT v.name, v.direction  FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE v.direction = 'backend' AND c.name LIKE '%Yandex%'
SELECT c.name  FROM companies c JOIN vacancies v ON c.company_id = v.company_id WHERE c.company_city = 'Москва' AND v.level = 'senior'
...
SELECT AVG(v.salary_from)  FROM vacancies v JOIN companies c ON v.company_id = c.company_id JOIN cities ci ON c.company_city = ci.city_name WHERE ci.city_name = 'Екатеринбург'
