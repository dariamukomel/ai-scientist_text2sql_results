...
SELECT DISTINCT(vacancies.work_location)  FROM vacancies  WHERE vacancies.schedule_type = 'remote'  AND vacancies.work_location IS NOT NULL
...
SELECT COUNT(*)  FROM vacancies  JOIN companies ON vacancies.company_id = companies.company_id  WHERE vacancies.level = 'middle'  AND companies.company_city = 'Санкт-Петербург'  AND vacancies.date_added >= '2022-01-01'  AND vacancies.date_added < '2023-01-01'
