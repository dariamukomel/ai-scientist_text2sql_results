...
SELECT vacancies.schedule_type, AVG((vacancies.salary_from  vacancies.salary_to)/2) AS avg_salary  FROM vacancies  GROUP BY vacancies.schedule_type
...
SELECT COUNT(*)  FROM vacancies  WHERE vacancies.level = 'middle'  AND vacancies.salary_from > 100000  AND vacancies.salary_currency = 'RUR'
...
SELECT vacancies.name  FROM vacancies  ORDER BY (vacancies.salary_from  vacancies.salary_to)/2 DESC  LIMIT 10
