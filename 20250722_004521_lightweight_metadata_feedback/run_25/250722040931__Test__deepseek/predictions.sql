...
SELECT      vacancies.work_location,     MAX(vacancies.salary_to) AS max_salary FROM      vacancies WHERE      vacancies.work_location IN ('Москва', 'Омск', 'Екатеринбург')     AND vacancies.date_added >= '2022-10-01'     AND vacancies.date_added < '2022-11-01' GROUP BY      vacancies.work_location
...
SELECT * FROM vacancies WHERE vacancies.name = 'C++ разработчик' AND (vacancies.salary_from > 200000 OR vacancies.salary_to > 200000)
...
SELECT * FROM vacancies  WHERE salary_from >= 50000 AND salary_from <= 100000 AND salary_currency = 'RUR'
SELECT DISTINCT(department_code) FROM departments
...
SELECT * FROM vacancies  WHERE vacancies.work_location = 'Санкт-Петербург'  AND vacancies.date_added >= '2021-02-01'  AND vacancies.date_added < '2021-03-01'
...
SELECT COUNT(DISTINCT(vacancies.company_id))  FROM vacancies  WHERE vacancies.schedule_type = 'fullDay'
...
SELECT vacancies.vacancy_id, vacancies.name  FROM vacancies  JOIN companies ON vacancies.company_id = companies.company_id  WHERE vacancies.direction = 'backend' AND companies.name LIKE '%Yandex%'
...
SELECT COUNT(*)  FROM vacancies  WHERE vacancies.level = 'middle'  AND vacancies.work_location = 'Санкт-Петербург'  AND vacancies.date_added >= '2022-01-01'  AND vacancies.date_added < '2023-01-01'
SELECT AVG(vacancies.salary_from)  FROM vacancies  JOIN companies ON vacancies.company_id = companies.company_id  WHERE companies.company_city = 'Екатеринбург'
