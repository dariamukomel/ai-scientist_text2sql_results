...
SELECT COUNT(*) FROM vacancies WHERE level = 'middle' AND (salary_from > 100000 OR salary_to > 100000);
...
SELECT vacancies.work_location, COUNT(*) AS number_of_vacancies FROM vacancies GROUP BY vacancies.work_location ORDER BY number_of_vacancies DESC LIMIT 1;
...
SELECT *  FROM vacancies  WHERE salary_currency = 'USD' AND salary_from > 5000;
...
SELECT DISTINCT vacancies.name AS VacancyName FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.name = 'ООО Ништяк';
SELECT *  FROM vacancies  WHERE date_added > '2022-01-01';
SELECT * FROM vacancies ORDER BY salary_to DESC LIMIT 10;
SELECT departments.department_code, departments.department_name FROM vacancies JOIN departments ON vacancies.department_id = departments.department_id WHERE vacancies.vacancy_id = 3;
SELECT vacancies.name AS VacancyName FROM vacancies JOIN departments ON vacancies.department_id = departments.department_id JOIN companies ON departments.company_id = companies.company_id WHERE companies.name = 'Yandex' AND vacancies.direction = 'backend';
...
SELECT v.name AS Vacancy_Name, c.name AS Company_Name FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_city = 'Санкт-Петербург' AND v.direction = 'bigdata';
