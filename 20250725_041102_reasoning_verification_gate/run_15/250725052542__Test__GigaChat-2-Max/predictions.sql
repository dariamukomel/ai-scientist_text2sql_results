...
SELECT name, salary_from, salary_to FROM vacancies WHERE salary_from >= 50000 AND salary_to <= 100000 AND salary_currency = 'RUR';
...
SELECT work_location, COUNT(*) AS number_of_vacancies FROM vacancies GROUP BY work_location ORDER BY number_of_vacancies DESC LIMIT 1;
...
SELECT name, level, salary_from, salary_currency FROM vacancies WHERE salary_from > 5000 AND salary_currency = 'USD';
...
SELECT COUNT(*) FROM vacancies WHERE level = 'middle' AND work_location = 'Санкт-Петербург' AND date_added >= '2022-01-01' AND date_added < '2023-01-01';
