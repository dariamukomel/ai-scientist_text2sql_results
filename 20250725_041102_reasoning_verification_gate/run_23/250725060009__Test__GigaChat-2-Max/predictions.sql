...
SELECT vacancies.work_location AS city, COUNT(*) AS number_of_vacancies FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE vacancies.name LIKE '%Python разработчик%' GROUP BY vacancies.work_location ORDER BY number_of_vacancies DESC LIMIT 1;
...
SELECT vacancies.* FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.company_city = 'Омск' AND vacancies.schedule_type = 'flexible';
