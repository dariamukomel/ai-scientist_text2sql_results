...
SELECT work_location, COUNT(*) AS num_vacancies FROM vacancies WHERE name LIKE '%Python разработчик%' GROUP BY work_location ORDER BY num_vacancies DESC LIMIT 1;
...
SELECT name, work_location, salary_from, salary_to, level FROM vacancies WHERE salary_currency = 'USD';
SELECT COUNT(*) FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.name = 'ООО Ромашка';
...
SELECT COUNT(*) FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE v.level = 'middle' AND c.company_city = 'Санкт-Петербург' AND v.date_added >= '2022-01-01' AND v.date_added <= '2022-12-31';
...
SELECT v.name AS Vacancy_Name, v.level AS Level, v.salary_from AS Salary_From, v.salary_to AS Salary_To FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_city = 'Санкт-Петербург' AND v.direction = 'bigdata';
