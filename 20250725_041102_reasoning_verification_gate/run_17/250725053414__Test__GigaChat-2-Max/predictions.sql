...
SELECT name, salary_from, salary_to, work_location  FROM vacancies  WHERE salary_currency = 'USD';
...
SELECT COUNT(vacancies.vacancy_id) FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.company_city = 'Москва' AND vacancies.schedule_type = 'remote';
...
SELECT v.name AS VacancyName, c.name AS CompanyName FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_city = 'Санкт-Петербург' AND v.direction = 'bigdata';
