...
SELECT      work_location AS city,     MAX(salary_to) AS max_salary FROM      vacancies WHERE      work_location IN ('Москва', 'Омск', 'Екатеринбург') AND      date_added BETWEEN '2022-10-01' AND '2022-10-31' GROUP BY      work_location;
...
SELECT work_location, COUNT(*) as count_vacancies FROM vacancies WHERE name LIKE '%Python разработчик%' GROUP BY work_location ORDER BY count_vacancies DESC LIMIT 1;
...
SELECT vacancies.vacancy_id, vacancies.name FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.company_city = 'Омск' AND vacancies.schedule_type IN ('flexible', 'remote');
...
SELECT MAX(salary_to) AS max_salary_in_moscow FROM vacancies WHERE work_location = 'Москва';
SELECT COUNT(*) FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE v.salary_from BETWEEN 50000 AND 100000 AND c.name = 'ООО Рога и Копыта';
SELECT *  FROM vacancies  WHERE date_added = '2020-04-01';
SELECT COUNT(*)  FROM vacancies  WHERE date_added > '2021-01-01';
SELECT name, salary_from, salary_to  FROM vacancies  WHERE salary_from BETWEEN 50000 AND 100000  AND salary_currency = 'RUR';
...
SELECT COUNT(*) AS number_of_vacancies FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.name = 'Yandex';
...
SELECT vacancies.name AS VacancyName FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.name = 'ООО Ништяк';
...
SELECT COUNT(*) FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE v.level = 'middle' AND v.date_added >= '2022-01-01' AND v.date_added <= '2022-12-31' AND c.company_city = 'Санкт-Петербург';
...
SELECT COUNT(*) FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_street ILIKE '%улица Ленина%';
SELECT vacancies.name AS VacancyName, companies.name AS CompanyName FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.company_city = 'Санкт-Петербург' AND vacancies.direction = 'bigdata';
