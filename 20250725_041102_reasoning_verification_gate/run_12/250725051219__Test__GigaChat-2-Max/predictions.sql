...
SELECT COUNT(*)  FROM vacancies  WHERE name = 'C++ разработчик' AND salary_from > 200000;
...
SELECT salary_currency, MAX(salary_to) AS max_salary FROM vacancies WHERE salary_to IS NOT NULL GROUP BY salary_currency;
...
SELECT name, work_location, salary_from, salary_to, schedule_type FROM vacancies WHERE salary_currency = 'USD';
...
SELECT vacancies.name AS "Название вакансии", companies.name AS "Компания" FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.company_city = 'Санкт-Петербург' AND vacancies.direction = 'bigdata';
