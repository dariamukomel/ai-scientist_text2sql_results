...
SELECT      work_location AS Город,     MAX(salary_to) AS Максимальная_зарплата FROM      vacancies WHERE      work_location IN ('Москва', 'Омск', 'Екатеринбург') AND      date_added BETWEEN '2022-10-01' AND '2022-10-31' GROUP BY      work_location;
...
SELECT *  FROM vacancies  WHERE work_location = 'Москва'  ORDER BY salary_to DESC  LIMIT 5;
...
SELECT DISTINCT companies.name AS Company_Name FROM companies JOIN vacancies ON companies.company_id = vacancies.company_id WHERE companies.company_city = 'Москва' AND vacancies.level = 'senior';
