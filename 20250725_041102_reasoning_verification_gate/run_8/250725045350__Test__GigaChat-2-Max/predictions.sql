...
SELECT vacancies.work_location, COUNT(*) AS count_vacancies FROM vacancies WHERE vacancies.name LIKE '%Python разработчик%' GROUP BY vacancies.work_location ORDER BY count_vacancies DESC LIMIT 1;
...
SELECT vacancies.name, vacancies.salary_to  FROM vacancies  WHERE vacancies.work_location = 'Москва'  ORDER BY vacancies.salary_to DESC  LIMIT 5;
SELECT vacancy_id, name, date_added FROM vacancies ORDER BY date_added DESC LIMIT 10;
SELECT COUNT(*)  FROM vacancies  WHERE work_location = 'Екатеринбург'  AND date_added >= '2022-01-01'  AND date_added <= '2022-01-31';
