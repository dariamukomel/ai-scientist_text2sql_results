...
SELECT      work_location AS "Город",     MAX(salary_to) AS "Максимальная зарплата" FROM      vacancies WHERE      work_location IN ('Москва', 'Омск', 'Екатеринбург') AND      date_added BETWEEN '2022-10-01' AND '2022-10-31' GROUP BY      work_location;
...
SELECT salary_currency, MAX(salary_to) AS max_salary FROM vacancies WHERE salary_to IS NOT NULL AND salary_to != 0 GROUP BY salary_currency;
SELECT vacancies.vacancy_id, vacancies.name, vacancies.schedule_type FROM vacancies JOIN companies ON vacancies.company_id = companies.company_id WHERE companies.company_city = 'Омск' AND vacancies.schedule_type = 'mixed';
