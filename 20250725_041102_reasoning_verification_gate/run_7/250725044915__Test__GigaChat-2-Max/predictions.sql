...
SELECT work_location AS Город, MAX(salary_to) AS Максимальная_зарплата FROM vacancies WHERE work_location IN ('Москва', 'Омск', 'Екатеринбург') AND date_added >= '2022-10-01' AND date_added < '2022-11-01' GROUP BY work_location;
...
SELECT vacancies.name AS "Название вакансии",        vacancies.salary_from AS "Минимальная зарплата",        vacancies.salary_to AS "Максимальная зарплата",        vacancies.salary_currency AS "Валюта",        vacancies.work_location AS "Место работы",        vacancies.date_added AS "Дата публикации" FROM vacancies WHERE vacancies.salary_from > 5000 AND vacancies.salary_currency = 'USD';
