...
SELECT work_location, COUNT(*) AS num_vacancies FROM vacancies WHERE name LIKE '%Python%' GROUP BY work_location ORDER BY num_vacancies DESC LIMIT 1;
SELECT COUNT(*)  FROM vacancies  WHERE salary_from IS NULL AND salary_to IS NULL;
SELECT vacancies.salary_currency, MAX(vacancies.salary_to) FROM vacancies GROUP BY vacancies.salary_currency;
...
SELECT name, work_location, level, direction FROM vacancies WHERE salary_currency = 'USD' AND salary_from > 5000;
