...
SELECT vacancies.work_location, COUNT(*) AS num_vacancies FROM vacancies WHERE vacancies.name LIKE '%Python%' GROUP BY vacancies.work_location ORDER BY num_vacancies DESC LIMIT 1;
...
SELECT COUNT(*) AS count_of_jobs FROM vacancies v JOIN companies c ON v.company_id = c.company_id WHERE c.company_city = 'Омск' AND v.salary_currency = 'USD';
...
SELECT vacancies.name AS VacancyName, vacancies.salary_to AS SalaryTo FROM vacancies WHERE vacancies.work_location = 'Москва' ORDER BY vacancies.salary_to DESC LIMIT 5;
...
SELECT name, salary_from  FROM vacancies  WHERE salary_currency = 'USD' AND salary_from > 5000;
...
SELECT v.vacancy_id, v.name, d.department_name FROM vacancies AS v JOIN departments AS d ON v.department_id = d.department_id JOIN companies AS c ON d.company_id = c.company_id WHERE c.name = 'Yandex' AND v.direction = 'backend';
