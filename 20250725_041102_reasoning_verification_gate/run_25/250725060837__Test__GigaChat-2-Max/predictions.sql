...
SELECT vacancies.work_location, COUNT(*) AS num_vacancies FROM vacancies WHERE vacancies.name LIKE '%Python разработчик%' GROUP BY vacancies.work_location ORDER BY num_vacancies DESC LIMIT 1;
...
SELECT departments.department_name, departments.department_code FROM vacancies JOIN departments ON vacancies.department_id = departments.department_id WHERE vacancies.vacancy_id = 3;
...
SELECT COUNT(vacancies.vacancy_id) FROM vacancies JOIN departments ON vacancies.department_id = departments.department_id JOIN companies ON departments.company_id = companies.company_id WHERE companies.name = 'ООО Рога и Копята' AND departments.department_name = 'finance';
